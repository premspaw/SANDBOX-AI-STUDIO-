...
export const SonicDock = () => {
    // Optimization: Use granular selectors to prevent unnecessary re-renders when other store fields change
    ...
    const [narrative, setNarrative] = useState(''); // Fix: Added missing state to resolve reference error in JSX
    const mouseX = useMotionValue(Infinity);

    // Optimization: Memoize tools array to avoid recreation on every render
    const tools = useMemo(() => [
    ...
    ], [addInfluencerNode, addSeedanceNode, addSeedance15ProNode, addNanoBananaNode]);
