...
export function InfluencerStudio({ setActiveTab }) {
    ...
    const fetchCharactersFromSupabase = async () => {
        try {
            ...
            if (charResp && charResp.ok) {
                ...
                if (proxyChars.length > 0) {
                    const formatted = proxyChars.map(c => ({
                        ...
                        photo: c.anchor_image || c.image || c.photo || '',
            ...
            // 2. Fallback to direct supabase only if proxy fails or is empty
            if (supabase) {
                ...
                try {
                    ...
                    console.warn("InfluencerStudio: Direct Supabase fetch failed:", supErr.message);
                }
            }
        } catch (e) {
        ...
        if (mode === 'image') {
            ...
            try {
                ...
                if (data.url) {
                    ...
                    window.toast(data.message || data.error || 'Generation failed');
                }
            } catch (e) {
                window.toast('Connection error: ' + e.message);
            }

        } else if (mode === 'video') {
            if (!output?.url || output.type === 'video') {
                window.toast('Generate a base image first, then switch to UGC Video mode to animate it.');
                setLoading(false);
                return;
            }
            ...
            try {
                ...
                if (data.url) {
                    ...
                    window.toast(data.error || 'Video generation failed');
                }
            } catch (e) {
                window.toast('Connection error: ' + e.message);
            }
    ...
    const handleGenerateKit = async () => {
        ...
        try {
            ...
            if (kitData) {
                ...
                window.toast(data.error || 'Identity kit generation failed. Try generating images individually.');
            }
        } catch (e) {
            window.toast('Connection error: ' + e.message);
        } finally {
