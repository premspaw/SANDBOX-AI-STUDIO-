...
import { Eye, EyeOff, Camera, Sun, Music, Zap, Wind, MessageSquare, Film, Layers, Play, Loader } from 'lucide-react';
                ...
                {isVisible && assembledPrompt && (
                    ...
                    >
                        ...
                        <div className="px-4 py-3 border-t border-white/5 bg-black/20">
                            ...
                            {(() => {
                                // Calculate final prompt outside the JSX so we can use it
                                ...
                                if (!promptText) {
                                    ...
                                    promptText = `${style} shot of ${char} ${subjectAction}.`;
                                }
                                ...
                                const handleGenerate = async () => {
                                    ...
                                    try {
                                        const url = await generateCharacterImage({
                                            prompt: promptText,
                                            modelEngine: 'nano-banana-2',
                                            references: [activeCharacter?.identityKit?.anchor, activeCharacter?.identityKit?.matrix].filter(Boolean),
                                            quality: '2k'
                                        });
                                        setPreviewImage(url);
                                    } catch (error) {
                                    ...
                                    <div className="flex flex-col gap-3">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <button
                                                ...
                                                className="flex-1 flex items-center justify-center gap-2 px-3 py-1.5 bg-[#bef264]/10 hover:bg-[#bef264]/20 border border-[#bef264]/30 rounded text-[#bef264] text-[9px] font-black uppercase tracking-widest transition-colors disabled:opacity-50"
                                            >
                                                {isGenerating ? <Loader className="animate-spin" size={12} /> : <Play size={12} />}
                                                {isGenerating ? 'GENERATING...' : 'TEST NANO BANANA 2'}
                                            </button>
                                        </div>
                                        ...
                                        {previewImage && (
                                            <div className="mt-2 rounded-lg overflow-hidden border border-white/10 relative">
                                                ...
                                                <button
                                                    ...
                                                    className="absolute top-1 right-1 bg-black/50 text-white/50 hover:text-white rounded-full p-1 backdrop-blur"
                                                >
