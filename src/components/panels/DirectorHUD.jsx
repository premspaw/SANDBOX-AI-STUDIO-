...
const DirectorHUD = memo(() => {
    const {
        ...
        detailMatrix: s.detailMatrix,
        currentProduct: s.currentProduct,
        actionScript: s.actionScript,
        camera: s.camera,
        wardrobeImage: s.wardrobeImage,
        poseImage: s.poseImage,
        lastGeneratedPrompt: s.lastGeneratedPrompt,
        isRendering: s.isRendering,
        isSyncing: s.isSyncing,
        activeNodeId: s.activeNodeId,
        nodes: s.nodes,
        edges: s.edges,
        setWardrobeImage: s.setWardrobeImage,
        setPoseImage: s.setPoseImage,
        setMode: s.setMode,
        setRepairSession: s.setRepairSession,
        purgeVault: s.purgeVault,
        generateStoryboard: s.generateStoryboard,
        addNode: s.addNode,
        updateNodeData: s.updateNodeData,
        deleteNode: s.deleteNode,
        syncCurrentSession: s.syncCurrentSession,
        setState: s.setState
    })));
                ...
                {activeTab === 'UGC' && (
                    <div className="space-y-8">
                        <HUDSection title="UGC_VIRAL_PIPELINE" icon={Film}>
                            ...
                            <button
                                onClick={() => {
                                    const nodeId = useAppStore.getState().addUGCPipelineNode({ x: 300, y: 200 });
                            ...
                            <button
                                onClick={() => useAppStore.getState().addUGCEngineNode({ x: 500, y: 300 })}
                ...
                {activeTab === 'VISUAL' && (
                    <>
                        ...
                        <div className="flex gap-2 flex-wrap">
                            {[
                                { label: 'CAM', icon: Camera, fn: () => useAppStore.getState().addCameraNode() },
                                { label: 'LIGHT', icon: Sun, fn: () => useAppStore.getState().addLightingNode() },
                                { label: 'MUSIC', icon: Music, fn: () => useAppStore.getState().addMusicNode() },
                                { label: 'SFX', icon: Volume2, fn: () => useAppStore.getState().addSFXNode() },
                                { label: 'VOICE', icon: Mic2, fn: () => useAppStore.getState().addDialogueNode() },
                            ].map(btn => (
                                <button
                                    key={btn.label}
                                    onClick={btn.fn}
