...
import { MessageSquare, X, Mic2, Play, Loader2, Music2, Wand2 } from 'lucide-react';
import { useAppStore } from '../../store';
import { getApiUrl } from '../../config/apiConfig';
import { suggestDialogueAlternatives } from '../../services/geminiService';
...
export default memo(({ id, data }) => {
    ...
    const handleSynthesize = async () => {
        ...
        try {
            ...
            if (result.audioContent) {
                ...
                window.toast(`Voice Generation Failed: ${result.error || 'Unknown error'}`);
            }
        } catch (err) {
            console.error("Synthesis error:", err);
            window.toast("Connection error: Make sure the local server is running on port 3002.");
        } finally {
    ...
    const handleSuggestAlternatives = async () => {
        if (!data.script || isSuggesting) return;
        setIsSuggesting(true);
        try {
            const alts = await suggestDialogueAlternatives(data.script);
    ...
    const applyAlternative = (altText) => {
        updateNodeData(id, { script: altText });
        setAlternatives([]);
    };
    ...
    return (
        <div className="relative group" style={{ zIndex: 1 }}>
            <motion.div
                ...
                className="px-5 py-4 bg-[#0a0a0a]/90 border-2 border-purple-500/20 rounded-2xl min-w-[250px] shadow-[0_20px_40px_rgba(0,0,0,0.4)] hover:border-purple-500/50 transition-all font-sans"
            >
                ...
                <AnimatePresence>
                    {alternatives.length > 0 && (
                        ...
                        >
                            <span className="text-[8px] text-purple-400 font-bold uppercase block mb-1">Suggested Alternatives:</span>
                            {alternatives.map((alt, idx) => (
                                <button
                                    key={idx}
                                    onClick={() => applyAlternative(alt)}
                                    className="w-full text-left p-2 bg-purple-500/10 hover:bg-purple-500/20 rounded-lg text-[9px] text-white/70 italic border border-purple-500/10 transition-colors"
                                >
                                    "{alt}"
                                </button>
                            ))}
                ...
                <div className="mt-4 flex items-center justify-between gap-2">
                    <button
                        ...
                        disabled={isSuggesting || !data.script}
                        className="py-1.5 px-3 bg-fuchsia-600/20 hover:bg-fuchsia-600/40 disabled:opacity-50 rounded-lg border border-fuchsia-500/30 text-[9px] font-bold text-white transition-all flex items-center justify-center gap-1.5"
                        title="Suggest Alternative Phrasing"
                    >
