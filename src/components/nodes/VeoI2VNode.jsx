...
export default memo(({ id, data }) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            if (result.videoUrl) {
                ...
                setStatusMsg('');
                try {
                    const { saveGeneratedAsset } = await import('../../services/supabaseService');
                    saveGeneratedAsset(result.videoUrl, 'video', `veo_${id}_${Date.now()}.mp4`);
                } catch (_) { }
            } else {
                setStatusMsg('');
                window.toast(`Video Failed: ${result.error || 'Unknown error'}`);
            }
        } catch (err) {
    ...
    // ── Continuity Checker (Gemini 2.5 Pro Video) ───────────────────────
    const handleAnalyzeContinuity = async () => {
        if (isAnalyzing || !data.videoUrl) return;
        setIsAnalyzing(true);
        setContinuityReport(null);
        try {
            const { analyzeVideoContinuity } = await import('../../services/geminiService');
            const bible = useAppStore.getState().universeBible;
            const report = await analyzeVideoContinuity(data.videoUrl, bible);
                ...
                <AnimatePresence>
                    {data.videoUrl && (
                        ...
                        >
                            ...
                            <div className="absolute inset-x-0 bottom-0 p-2 bg-gradient-to-t from-black/90 to-transparent opacity-0 group-hover/vid:opacity-100 transition-opacity flex items-center justify-between">
                                ...
                                <div className="flex gap-1.5">
                                    <button
                                        ...
                                        className="p-1.5 bg-purple-500/80 text-white rounded-lg shadow-xl hover:scale-110 active:scale-95 transition-all text-[8px] font-black"
                                        title="Continuity Check (Gemini 2.5 Pro)"
                                    >
                                        {isAnalyzing ? <Loader2 size={9} className="animate-spin" /> : '🔍'}
                                    </button>
                                    <button
                            ...
                            {/* Continuity Report */}
                            {continuityReport && (
                                <div className="absolute inset-x-2 top-2 p-2 bg-black/80 backdrop-blur-md rounded border border-purple-500/30 text-[8px] text-purple-200 font-mono shadow-2xl max-h-[80%] overflow-y-auto">
                                    <div className="font-bold text-purple-400 mb-1 flex justify-between">
                                        <span>DIRECTOR'S CONTINUITY CHECK</span>
                                        <button onClick={() => setContinuityReport(null)} className="opacity-50 hover:opacity-100"><X size={10} /></button>
                                    </div>
                                    <div className="whitespace-pre-wrap">{continuityReport}</div>
                                </div>
                            )}
                        </motion.div>
                    )}
