...
export default memo(({ id, data }) => {
    ...
    const handleGenerateStoryboard = async () => {
        if (hasBlockingDependencies) {
            ...
            window.toast(`⚠️ Please wait for: ${blockedNodes}`);
            return;
        }

        // Validate context mode
        if (contextMode === 'empty') {
            window.toast('⚠️ Please connect at least a Character or Product node.');
            return;
        }
        ...
        try {
            ...
            window.toast(`❌ Generation failed: ${err.message}`);
        } finally {
