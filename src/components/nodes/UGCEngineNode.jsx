...
const UGCEngineNode = ({ id, data }) => {
    ...
    const handlePreviewVideo = async (sceneIndex = 0) => {
        if (!characterImage || !productImage) {
            window.toast('Character and Product images are required to generate a preview.');
            return;
        }

        const targetScene = results.scenes[sceneIndex];
        if (!targetScene) {
            window.toast('Target scene not found.');
            return;
        }
    ...
    const handleCompileAd = async () => {
        ...
        if (renderedScenes.length === 0) {
            window.toast("Protocol Violation: No rendered sequences found. Render at least one scene first.");
            return;
        }
        ...
        try {
            ...
            window.toast(`Compilation Failed: ${error.message}`);
