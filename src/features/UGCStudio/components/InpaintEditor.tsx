...
import { X, Pencil, Upload, Camera, Loader2 } from 'lucide-react';
...
export default function InpaintEditor({
  ...
  const { showToast, handleApiError } = useUGC();
  ...
  const handleEdit = async () => {
    if (!instruction.trim()) {
      showToast('Describe what to change in the painted area.', 'error');
      return;
    }
  ...
  if (isEmbedded) {
    return (
      <div className="relative w-full h-full bg-[#0e0e11] border border-white/10 rounded-2xl overflow-hidden flex flex-col flex-1 min-h-0">
        {/* Header */}
          ...
          <div className="flex-1 relative overflow-auto flex items-center justify-center bg-black/40 p-3">
            <div className="relative inline-block">
              <img
                ...
                alt="edit base"
                onLoad={initCanvas}
                className="block rounded-xl max-w-full max-h-[65vh] object-contain"
              />
              <canvas
                ref={canvasRef}
            ...
            <div className="space-y-2">
              ...
              {refImage ? (
                <div className="relative rounded-xl overflow-hidden border border-lime-500/30 bg-white/5">
                  <img src={resolveUrl(refImage)} alt="ref" className="w-full max-h-24 object-cover" />
                  <button
