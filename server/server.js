...
// Polyfill for Node.js (fetch and File)
if (!globalThis.fetch) {
...
async function handleGoogle(req, res) {
    try {
        ...
        if (targetModel === 'veo') {
            ...
            const modelName = modelMapping[targetModel] || 'gemini-2.5-flash-image';
            ...
            const { images = [], references = [], consistencyRefs = [] } = req.body;
            const combinedRefs = [...images, ...references, ...identity_images, ...consistencyRefs, product_image].filter(Boolean);
            ...
            // Standard payload for Gemini Image Generation API
            const payload = {
